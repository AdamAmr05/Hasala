...
import { Transaction, TransactionType, Category, SavingsGoal } from '../../types';
import {
...
import {
  ...
  ArrowDownLeft,
  Repeat,
  PiggyBank, Smartphone, Plane, Laptop, Gift, Wallet, Target
} from 'lucide-react';
...
const ChatWidget: React.FC<ChatWidgetProps> = React.memo(({ type, transactions, budget, data: propsData }) => {
  ...
  // 8. Savings Overview Widget
  if (type === 'renderSavingsOverview') {
    const savingsData = propsData?.savings;
    if (!savingsData) return null;

    const { totalSaved: savingsTotal, totalTarget: savingsTarget, overallProgress: savingsProgress, topGoals: savingsTopGoals } = savingsData;

    const resolveGoalIcon = (icon: string, name: string) => {
      // 1. Priority: User's selected icon (usually an emoji)
      if (icon && /\p{Emoji}/u.test(icon)) {
        return <span className="text-lg">{icon}</span>;
      }

      // 2. Fallback: Smart keyword matching if no valid icon found
    ...
    return (
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-500 dark:text-gray-400 font-medium mt-0.5">
                You've saved <span className="text-emerald-600 dark:text-emerald-400 font-bold">{Math.round(savingsProgress)}%</span> of your goals
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="relative w-16 h-16 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90">
                ...
                <circle
                  ...
                  strokeDashoffset={163.3 - (163.3 * savingsProgress) / 100}
                  className="text-white transition-all duration-1000 ease-out"
                  strokeLinecap="round"
                />
              </svg>
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-xs font-bold">{Math.round(savingsProgress)}%</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5">
          ...
          {savingsTopGoals.length > 0 ? (
            savingsTopGoals.map((goal: SavingsGoal, index: number) => (
              <div key={index} className="group">
                <div className="flex justify-between items-center mb-2.5">
                  <div className="flex items-center gap-3.5">
                    ...
                    >
                      {resolveGoalIcon(goal.icon, goal.name)}
                    </div>
                    <div>
